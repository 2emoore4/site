...
public class Renderer {
    ...
    private void renderGeometry(Geometry geo) {
        ...
        if (geo.hasVertex()) {
            for (int f = 0 ; f < geo.getFaces().length; f++) {
                for (int e = 0; e < geo.getFace(f).length - 1; e++) {
                    int i = geo.getFace(f)[e];
                    int j = geo.getFace(f)[e + 1];

                    geo.getGlobMatrix().transform(geo.getVertex(i), point0);
                    geo.getGlobMatrix().transform(geo.getVertex(j), point1);
                ...
                int i = geo.getFace(f)[geo.getFace(f).length - 1];
                int j = geo.getFace(f)[0];
